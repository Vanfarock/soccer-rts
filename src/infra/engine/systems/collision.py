...
from infra.engine.primitives.game_object import GameObject
...
def _resolve_solids(colliders: list[ColliderComponent]) -> None:
    ...
    for index, first in enumerate(solids):
        for second in solids[index + 1 :]:
            ...
            if (
                first.shape is ColliderShape.CIRCLE
                and second.shape is ColliderShape.CIRCLE
            ):
                _resolve_circle_circle(first, second)
            elif (
                first.shape is ColliderShape.CIRCLE
                and second.shape is ColliderShape.RECT
            ):
                _resolve_circle_rect(first, second)
            elif (
                first.shape is ColliderShape.RECT
                and second.shape is ColliderShape.CIRCLE
            ):
