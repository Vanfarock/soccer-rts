...
class PlayerMovementComponent(Component):
    ...
    def update(self, ctx: FrameContext) -> None:
        ...
        if isinstance(owner, Player):
            if not owner.focused:
                return

            controller = owner.get_component(KeyboardControllerComponent)
            if controller is not None:
                if ctx.pressed_mouse[2] and PossessionService.has_ball(
                    owner, controller.ball
                ):
                    return
                speed = PossessionService.movement_speed(owner, controller.ball)
